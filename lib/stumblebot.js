...
const send = (msg, to) => {
    xmpp.send(xml("message", { to, from: 'stumblechat@' + process.env.STUMBLEBOT_XMPP_DOMAIN, id: xid(), type: "chat" }, xml("body", {}, msg)));
};
const cursors = {};
...
const handleCommand = async (body, _to) => {
    ...
    if (tokens[0] === "/set-proxy") {
        ...
        await stumble
            ...
            handler(v) {
                ...
                if ((v.stumble === 'msg' || v.stumble === 'pvtmsg') && v.handle)
                    send((users[v.handle] || v.handle) + (v.stumble === 'pvtmsg' ? '::<private>' : '') + ':: ' + v.text, to);
                if (v.stumble === 'join')
                    associate(v);
                if (v.stumble === 'joined')
                    v.userlist.forEach((v) => associate(v));
            },
        })
        ...
        if (!socket)
            send('room not found ' + tokens[1], to);
        ...
        stumble.send(JSON.stringify({ stumble: 'pvtmsg', handle: Object.entries(users).find(([k, v]) => v.split('|')[0] === tokens[1])[0], text: body.substr(tokens.slice(0, 2).join(' ').length) }));
    }
    else if (tokens[0][0] === '/') {
