...
async function saveSessions() {
    await fs.writeFile(path.join(process.env.HOME, '.stumblebot', 'sessions.json'), JSON.stringify(Object.entries(stumbleSessions).map(([k, v]) => [k, { ...v.toObject(), proxyOptions: v.proxyOptions }]), null, 2));
}
;
async function loadSessions() {
    try {
        return Object.fromEntries(JSON.parse(await fs.readFile(path.join(process.env.HOME, '.stumblebot', 'sessions.json'), 'utf8')).map(([k, v]) => [k, Object.assign(Stumblechat.fromObject(v), { proxyOptions: v.proxyOptions })]));
    }
    catch (e) {
        return {};
    }
...
const handleCommand = async (body, _to) => {
    ...
    const to = split.length > 1 ? split.slice(0, split.length - 1).join("/") : split[0];
    ...
    if (tokens[0] === "/set-proxy") {
            ...
            await stumble
                ...
                handler(v) {
                    if (typeof v === "object") {
                        ...
                        appendLog(JSON.stringify(v, null, 2)).catch((err) => console.error(err));
                    }
                    if (v.stumble === "sysmsg")
                        send(v.text, to);
                    if ((v.stumble === "msg" || v.stumble === "pvtmsg") && v.handle)
                        send((users[v.handle] || v.handle) +
        ...
        stumble.send(JSON.stringify({
            stumble: "pvtmsg",
            handle: Object.entries(users).find(([k, v]) => v.split("|")[0] === tokens[1])[0],
...
exports.run = run;
//# sourceMappingURL=stumblebot.js.map